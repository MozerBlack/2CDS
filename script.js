...
// Seletores para Configurações (exemplo que já estava no seu JS)
const menuConfigLink = document.getElementById('menu-configuracoes'); // Este ID não estava no HTML, mas mantido no JS
...
// NOVOS seletores para Notícias
const menuNoticiasLink = document.getElementById('menu-noticias'); // NOVO: Link no menu
const noticiasPage = document.getElementById('noticias-page');    // NOVO: Div da página
const voltarHomeNoticiasButton = document.getElementById('voltar-home-noticias'); // NOVO: Botão Voltar
...
    configuracoesPage.classList.add('hidden-content'); 
    noticiasPage.classList.add('hidden-content'); // NOVO: Adiciona Notícias
}
...
// --- 5. Lógica de Navegação entre Home, Alunos, Notícias e Configurações ---
...
// Link 'Notícias'
if (menuNoticiasLink) {
    menuNoticiasLink.addEventListener('click', (event) => {
        event.preventDefault();
        hideAllPages();
        noticiasPage.classList.remove('hidden-content'); // Mostra Notícias
    });
}


// Link 'Configurações'
if (menuConfigLink) { // Verifica se o link existe (ele não está no HTML atual)
...
// Botão 'Voltar à Home' na página de notícias
if (voltarHomeNoticiasButton) {
    voltarHomeNoticiasButton.addEventListener('click', (event) => {
        event.preventDefault();
        showHomePage(VALID_USERNAME); // Usa a função de mostrar Home
    });
}
